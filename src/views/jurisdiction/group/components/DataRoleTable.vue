<template>
  <div>
    ...
    <el-table ref="tableData" :data="tableData" height="425px">
      ...
      <el-table-column label="权限" prop="privilege" width="80"> </el-table-column>
    ...
    <el-dialog v-if="dialogVisible" title="添加数据权限" :visible.sync="dialogVisible" width="85%" append-to-body :close-on-click-modal="false" @close="closeAddDataRole">
      <template #title>
        <span>{{ `添加数据权限${info.name ? ' - ' + info.name : ''}` }}</span>
      </template>
        ...
        <div class="selelectRole">
          <div class="title">
            ...
            <el-radio-group v-model="radio">
              <el-radio v-for="(item, index) in grantRole" :key="index" :label="item.label"></el-radio>
            </el-radio-group>
          </div>
        </div>
...
export default {
  ...
  data() {
    return {
      ...
      operation: ['描述表', '查询数据', '修改表', '插入数据', '删除表']
    };
  },
  computed: {
    ...mapGetters(['userInfo']),
  ...
  methods: {
    formatDataView(data) {
      return data ? data.toString().split('.').slice(1).join('.') : '-';
    },
    formatTableName(row) {
      if (row.grantedOn === 'CATALOG') {
        return this.formatDataView(row.uuid || row.name);
      } else {
        return this.formatDataView(row.name);
      }
    },
    closeDial() {
        ...
        .then(() => {
          ...
          if (type === 'row') {
            data.roleInputs.push({
              ...
              operation: params.privilege === '读取' ? operation.slice(0, 2) : operation,
            ...
            selectList.forEach(item => {
              roleinputs.push({
                ...
                operation: item.privilege === '读取' ? operation.slice(0, 2) : operation,
