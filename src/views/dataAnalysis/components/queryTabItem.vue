<template>
  <div class="query-tab-item">
      ...
      <div v-if="keyList.length > 0" class="variable">
        <div class="row">
          <div v-for="item in sqlKeyObj.keyList" :key="item.key" class="col">
            <div class="label">
              <el-tooltip class="item" effect="light" :content="item.key === 'execution_date' ? item.key + ' : ' + '系统默认的时间参数变量，只需定义该值，所有其它的jinjia变量值都会自动识别替换' : item.key" placement="top" popper-class="sqlParamsClass" :disabled="isTipDisabled">
                <span class="ellipsis block" @mouseenter="isShowTooltip">{{ item.key }}</span>
              </el-tooltip>
            </div>
            <el-date-picker v-if="item.key === 'execution_date'" v-model="item.value" type="datetime" size="mini" value-format="yyyy-MM-dd HH:mm:ss"> </el-date-picker>
            <el-input v-else v-model.trim="item.value" :disabled="!!item.disabled" placeholder="请输入参数值" :class="redKey.includes(item.key) ? 'redInput' : ''" size="mini"></el-input>
          </div>
        </div>
      </div>
    </div>
    <div v-if="unExecuteType" class="result">
      ...
      <result ref="result" :uuid="uuid" :data="result" :running="running" :engine="engine" :result-option="resultOption" :progress-obj="progressObj" :query-sql="querySql" @correct="changeAIsqlType" />
    </div>
...
export default {
  ...
  computed: {
    ...
    keyList() {
      ...
      if (regRes.length > 0) {
        ...
        // 符合jinjia表达式的处理
        if (res.some(val => jianjiaData.some(v => val.includes(v)))) {
          res = res.filter(item => {
            if (jianjiaData.some(v => item.includes(v))) {
              return !jianjiaData.some(str => {
                if (str === 'execution_date') return item.includes(str) || str === item;
                else return str === item;
              });
            }
            return item;
          });
          res.unshift('execution_date');
        }
  ...
  methods: {
    ...
    async farmatSqlFn(sql) {
      ...
      const keyList = this.sqlKeyObj.keyList.filter(
        item =>
          regRes.includes(item.key) &&
          !jianjiaData.some(str => {
            if (str === 'execution_date') return item.key.includes(str) || str === item.key;
            else return str === item.key;
          })
      );
      // jinja 表达式处理
