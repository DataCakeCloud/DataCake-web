  ...
  <div class="content_box">
    ...
    <div class="container">
      ...
      <div class="content_item_right">
        <div v-for="(item, i) in data.nodeList" :key="i" class="right_row">
            ...
            <el-tooltip v-for="(result, ii) in val" :key="ii" effect="dark" placement="right" popper-class="task_tootip" :disabled="!result.state">
              <template #content>
                <div class="tootip-content">
                  ...
                  <div>
                    <span>实例时间:</span>
                    <span>{{ result.executionDate ? $utils.parseTime(result.executionDate, '{y}-{m}-{d} {h}:{i}:{s}') : '-' }}</span>
                  </div>
                  <div>
                    <span>开始时间:</span>
                    <span>{{ result.startDate ? $utils.parseTime(result.startDate, '{y}-{m}-{d} {h}:{i}:{s}') : '-' }}</span>
                  </div>
                  <div>
                    <span>结束时间:</span>
                    <span>{{ result.endDate ? $utils.parseTime(result.endDate, '{y}-{m}-{d} {h}:{i}:{s}') : '-' }}</span>
                  </div>
                  <!-- <div>
                    <span>负责人:</span>
                ...
                <div slot="footer" class="tootip-footer" style="margintop: 10px">
                  ...
                  <el-button type="primary" @click="$emit('click-item', 'addNum', result, 1)">补 数</el-button>
                </div>
              </template>
              <span class="block" :style="nodeStyle(result)" @click="gotoDetail(result)"></span>
            </el-tooltip>
          </div>
        </div>
      </div>
...
export default {
  ...
  computed: {
    topData() {
      ...
      const listData = dataCopy.nodeList || [];
      ...
      const data = {};
      data.dataItemCom = {};
      const flatList = this.data.timeColumn.flat(Infinity);
      ...
      data.dataItemCom.list = this.data.timeColumn;
      const columnList = JSON.parse(JSON.stringify(this.data.timeColumn));

      const histogram = this.data.histogramStart.map(item => this.$utils.parseTime(item, '{y}/{m}/{d}'));
      // const histogram = this.$utils.parseTime(this.data.histogramStart, '{y}/{m}/{d}');
      ...
      data.dataItemCom.histogram_start = this.data.histogramStart.map(item => this.$utils.parseTime(item, '{y}/{m}/{d}'));
      data.avgTimeM = (sumData / dataItemAlllength).toFixed(0); // 平均时间高度
      data.avgTime = filters.duration(this.data.timeMedian * 1000); // 平均时间
      return data;
    }
  },
  methods: {
    ...
    nodeStyle(data) {
      ...
      if (data.state === 'waiting') {
        ...
        className = { backgroundColor: '#5b70e4' };
