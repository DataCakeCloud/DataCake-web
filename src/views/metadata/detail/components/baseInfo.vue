        ...
        <el-col :span="12">
          <p v-for="item in leftDetial" :key="item.prop" :class="['box-item', { pad: item.edit }]">
            ...
            <template v-if="editList.includes(item.prop)">
              ...
              <template v-else-if="item.prop === 'lifecycle'">
                <el-select v-model="tableInfo[item.prop]" class="item-trans" placeholder="请选择">
                  <el-option v-for="val in lifecycleOptions" :key="val.value" :label="val.label" :value="val.value"> </el-option>
                </el-select>
              </template>
            </template>
            <template v-else>
              ...
              <template v-else-if="item.prop === 'lifecycle'">
                <span class="value">
                  {{ formatLife(tableInfo[item.prop]) || '-' }}
                </span>
              </template>
              <!-- 正常展示 -->
...
const editorPropList = ['owner', 'securityLevel', 'description', 'useDirection', 'lifecycle'];
...
export default {
  ...
  data() {
    return {
      leftDetial: [
        ...
        { label: '生命周期', prop: 'lifecycle', edit: false },
        // { label: '行数', prop: 'count' }, //
  ...
  watch: {
    'tableInfo.listModel': function(data) {
      this.leftDetial = this.leftDetial.filter(item => this.defaultLeftDetial.some(val => val.prop === item.prop));
      data?.forEach(item => {
        // 模型有效 或者  无效但是之前选过的展示
  ...
  methods: {
    ...
    formatLife(val) {
      return this.lifecycleOptions.find(item => item.value === val)?.label || '-';
    },
    copy(str) {
      copy(str, {
        format: 'text/plain'
