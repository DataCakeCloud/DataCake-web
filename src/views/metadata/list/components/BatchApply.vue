...
style="width: 100%"
          @change="changeTable"
        >
          <el-option v-for="item in tableList" :key="item.table_name" :value="`${item.region}.${item.db_name}.${item.table_name}:${item.owner}:${item.id}`" :label="`${item.title}`" :disabled="item.tableAuth">
