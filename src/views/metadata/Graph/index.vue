<template>
  <div ref="flowContent" class="graph-content">
    <div :id="graphId"></div>
    <div v-if="isShowMinmap" :id="graphId + 'minmap'" class="minmap"></div>
  </div>
</template>
...
export default {
  ...
  created() {
    try {
      ...
      Graph.registerNode('table-node', {
        ...nodeConfig,
        component: TableNode
      });
      Graph.registerNode('column-node', {
        ...nodeConfig,
  ...
  methods: {
    init(width, height) {
      ...
      this.graph = new Graph({
        ...
        minimap: {
          enabled: true,
          container: document.getElementById(this.graphId + 'minmap')
        },
        grid: {
          size: 10, // 网格大小 10px
          visible: true // 渲染网格背景
    ...
    render(once) {
      ...
      once && this.graph.centerContent(); // 布局居中
    },
