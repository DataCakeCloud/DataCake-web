    ...
    <el-form ref="ruleForm" :model="ruleForm" :rules="rules" label-width="130px">
      ...
      <el-form-item v-if="ruleForm.type === 2" :label="$t('task.taskList35')" prop="childIds">
        ...
        <el-tree-select
          v-if="showTree"
          ref="treeSelect"
          v-model="ruleForm.childIds"
          v-loading="treeLoading"
          :placeholder="$t('task.taskList37')"
          class="dialog-select"
          :styles="{ width: '100%' }"
      ...
      <el-form-item v-if="ruleForm.type === 2" :label="$t('task.taskList39')" required>
  ...
  methods: {
    init(outputGuids) {
      ...
      getChildDependencies(this.currentId).then(res => {
        this.showTree = true;
        const data = res.data;
        this.$set(this.treeParams, 'data', this.filterData([data]));
