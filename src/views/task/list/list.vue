...
export default {
  ...
  methods: {
    ...
    copyTask(data) {
      ...
      let path;
      if (JSON.parse(data.runtimeConfig).executeMode === 'local' && data.templateCode === 'Mysql2Hive') {
        path = tools.taskCodeToPath[data.templateCode] + '2';
      } else {
        path = tools.taskCodeToPath[data.templateCode];
      }
      this.$router.push({
        path: `/task/${path}`,
        query: {
    ...
    gotoDetail(row) {
      const runtimeConfig = JSON.parse(row.runtimeConfig);
      if (row.isStreamingTemplateCode) {
        // 实时任务
        // const { href } = this.$router.resolve({
        //   path: '/task/info',
        ...
        this.$router.push({
          ...
          query: {
            id: row.id,
            executeMode: runtimeConfig.executeMode
          }
        });
      } else {
        // 离线任务
    ...
    edit(data) {
      ...
      //   }
      // });
      location.href = `/#/task/${path}?id=${data.id}`;
    },
    addNum(row) {
