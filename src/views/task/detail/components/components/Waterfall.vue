  ...
  <div>
    <ul class="waterfall">
      ...
      <li v-for="(task, i) in instances" :key="task.id" class="task-state">
        <el-tooltip v-if="task && task.start_date" class="item" effect="dark" placement="right" popper-class="task_tootip">
          <template #content>
            <div class="tootip-content">
              ...
              <div>
                <span>开始时间:</span>
                <span>{{ task.start_date ? $utils.parseTime(task.start_date, '{y}-{m}-{d} {h}:{i}:{s}') : '-' }}</span>
              </div>
              <div>
                <span>结束时间:</span>
                <span>{{ task.end_date ? $utils.parseTime(task.end_date, '{y}-{m}-{d} {h}:{i}:{s}') : '-' }}</span>
              </div>
              <!-- <div>
                <span>负责人:</span>
          ...
          <div :class="['instance', 'bar', 'task-state', task.state]" :style="{ left: `${getInstanceShitf(task.start_date)}px`, width: `${getInstanceWidth(task.start_date, task.end_date)}px` }" @click="handleInstance(i)"></div>
        </el-tooltip>
      </li>
    </ul>
  </div>
