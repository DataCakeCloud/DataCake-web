...
const service = axios.create({
  baseURL: process.env.NODE_ENV === 'development' ? process.env.VUE_APP_BASE_API7 : process.env.VUE_APP_API_PATH7, // url = base url + request url
...
service.interceptors.request.use(
  config => {
    ...
    if (store.getters['userInfo']) {
      config.headers['tenantId'] = store.getters['userInfo'].tenantId; // 给请求加上租户 ID
      config.headers['tenantName'] = store.getters['userInfo'].tenantName; // 给请求加上租户名称
      config.headers['userName'] = store.getters['userInfo'].userName; // 给请求加上租户 userName
    }
    return config;
  },
  error => {
