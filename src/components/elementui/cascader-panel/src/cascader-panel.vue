<template>
  <div :class="['el-cascader-panel', border && 'is-bordered']" @keydown="handleKeyDown">
    <div v-for="(menu, index) in menus" :key="index" class="el-scrollbar-box">
      <el-input v-model="searchKeyList[index]" size="mini" @input="changeSearch(searchKeyList[index], index)" class="search-ipt" prefix-icon="el-icon-search"></el-input>
      <cascader-menu ref="menu" :index="index" :nodes="menu"></cascader-menu>
    </div>
  </div>
</template>
  ...
  methods: {
    ...
    handleExpand(node, silent) {
      ...
      this.menus.forEach((item, itemIndex) => {
        if (!this.searchKeyList[itemIndex]) {
          this.initMenus[itemIndex] = item;
        }
      });
      if (!silent) {
    ...
    lazyLoad(node, onFullfiled) {
      ...
      if (!node) {
        ...
        this.menus = [this.store.getNodes()];
        this.initMenus = JSON.parse(JSON.stringify(this.menus));
      }
      node.loading = true;
